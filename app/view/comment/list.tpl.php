...
<span class='commentContent'><?=markdown($comment->comment)?></span>
...
<input type='submit' name='doCommentDelete' id='<?= $comment->id ?>' value='svara' title='svara' onclick='form.action="<?=$this->url->create('kommentar/svara/'.$comment->id)?>"'  title='<?=$comment->id?>'/>
<?php if ( isset( $userid ) && ($userid == 1 || $userid == 2 || $userid == $comment->user_id) ){ ?>
